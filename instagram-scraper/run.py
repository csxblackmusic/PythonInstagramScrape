...
async def run():
    ...
    """
    #user = await instagram.scrape_user("littlewaves.coffee")
    #output.joinpath("littlewaves.user.json").write_text(json.dumps(user, indent=2, ensure_ascii=False), encoding='utf-8')
    # scrape_user_posts requires the profile username
    posts_all = []
    async for post in instagram.scrape_user_posts("littlewaves.coffee"):
        posts_all.append(post)
    log.success("scraped {} posts", len(posts_all))
    output.joinpath("littlewaves-all-user-posts.json").write_text(json.dumps(posts_all, indent=2, ensure_ascii=False), encoding='utf-8')
